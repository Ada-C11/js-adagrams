...
drawLetters() {
    const letterPool = { A: 9, B: 2, C: 2, D: 4, E: 12, F: 2, G: 3, H: 2, I: 9, J: 1, K:1, L: 4, M: 2, N: 6, O: 8, P: 2, Q: 1, R: 6, S: 4, 
...
// Question for Dee. There has to be a more suscinct way of doing all of these conditionals, but I couldn't thing of one. 
// Would like to talk about it in our one on one.  
// learned about this for...of statement.  

for (let word of words) {
  if (word.length == 10 && winner['word'].length != 10) {
    winner['word'] = word;
    winner['score'] = this.scoreWord(word);
  } else if (this.scoreWord(word) >= winner['score'] && word.length == 10 && winner['word'].length != 10) {
    winner['word'] = word;
    winner['score'] = this.scoreWord(word);
  } else if (this.scoreWord(word) >= winner['score'] && word.length < winner['word'].length && winner['word'].length != 10) {
    winner['word'] = word;
    winner['score'] = this.scoreWord(word);
