const Adagrams = {
  drawLetters() {
    ...
    let letterArrayCopy = letterArray.slice();
    let randomIndex; 

    for (let i = 0; i < 10; i += 1){
      randomIndex = Math.floor ( Math.random() * letterArrayCopy.length)
      drawn.push(letterArrayCopy[randomIndex]);
      letterArrayCopy.splice(randomIndex, 1);
    }
    return drawn;
  },
